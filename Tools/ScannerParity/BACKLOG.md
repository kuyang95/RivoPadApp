...
- 영어·일본어에서 촬영 안내, 오류, 페이지 검토와 PDF 내보내기 문구 확인
...
상태: 자동 촬영·UVDoc·색상 강화 설정 연결 완료, 출력 값만 코드에 존재

- [x] 색상 강화 켜기/끄기와 다음 실행 복원
- [x] 자동 촬영 켜기/끄기, 기본 켜짐과 다음 실행 복원. 끈 상태에서도
  LCNet 안내·화면/Rivo 수동 촬영 유지
- [x] UVDoc 곡면 보정 켜기/끄기, 기본 켜짐과 다음 실행 복원. 끄면
  모델 준비·추론을 생략하고 원근 보정 결과로 안전하게 우회
- [x] 세 설정을 앱 설정 초기화에 포함
- [ ] 색상 강화 켠 결과와 끈 결과를 같은 문서로 M4에서 비교
- [ ] 자동 촬영 켬·끔에서 안정 프레임·수동 촬영·페이지 제거 대기와
  다음 페이지 동작을 실제 문서로 비교
- [ ] 같은 곡면 책 페이지를 UVDoc 켬·끔으로 촬영해 처리 시간·메모리와
  글자 휘어짐·잘림을 비교
- 출력 긴 변 길이 선택
- 촬영음·음성 안내 정책
...
상태: 단일 이미지와 다중 PDF→문서 OCR·로컬 교정 연결 완료, 내보내기 일부 남음

- OCR 원문과 영역 좌표가 회전·보정된 결과에 맞는지 검증
- [x] 스캔 결과에서 바로 AI 질문
- [x] Android 기본 켜짐 OCR 자동 교정을 정지 이미지와 다중 PDF의
  이미지 페이지에 연결하고 M4 Qwen3-VL이 이미지·Vision 원문을 함께 사용
- [x] 빈 결과, 16,000자 초과, 생성 실패·취소·AI 사용 중에는 원문 유지
- [x] 숫자·금액·날짜·전화·이메일·URL·줄바꿈 변경, 50% 초과 삭제,
  1.5배 초과 추가와 생성형 머리말·마크다운을 거절
- [ ] 한국어·영어·일본어 실제 인쇄물과 영수증에서 켬·끔 결과, 첫 모델
  준비 시간, 페이지당 교정 시간과 메모리 사용을 비교
- [ ] 원문보다 나빠진 교정, 숫자·고유명사 변경과 줄/영역 표시 불일치가
  없는지 확인하고 대표 이미지를 회귀 fixture로 보존
- 이미지 직접 저장·공유
...
- Rivo 촬영 화면의 4 닫기·7 촬영, 촬영 결과의 4 재촬영·9 페이지 추가·
  7 검토 완료·첫 페이지 0 한 장 OCR 구현
- Rivo 페이지 검토의 2/8 이전·다음 선택, 5 회전, 9 페이지 추가,
  0 문서 열기, 7 PDF 저장과 선택 행 표시 구현
- 외장 키보드 조작
