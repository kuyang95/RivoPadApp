...
- 사진·PDF·TXT·XLSX·XLS·HWP·HWPX 원본 또는 추출 문맥과 텍스트·URL
  본문은 설정한 M4 음성 질문/AI 채팅으로, EPUB은 독서로 라우팅
...
- Android처럼 공유 텍스트·웹페이지·사진·지원 문서 전체를 설정에 따라
  음성 질문 또는 AI 채팅으로 연다. 음성 질문은 공유 문맥·원본 준비 뒤
  온디바이스 듣기를 자동 시작하고, AI 채팅은 같은 자료를 저장한 새
  대화에서 질문을 기다린다.
...
- [ ] 사진 앱의 사진 한 장을 RivoPad로 공유하면 앱이 열리고 OCR 결과가
      아닌 설정한 음성 질문/AI 채팅에 원본 첨부로 표시되며, 첫 질문과
      후속 질문이 사진을 참조하고 앱 전환 실패 시 다음 실행에서 재시도된다.
- [ ] Files의 일반 PDF와 스캔 PDF를 공유하면 설정한 음성 질문/AI 채팅이
      열리고 내장 텍스트 추출 또는 페이지별 Vision OCR로 질문이 동작한다.
- [ ] Safari의 페이지 URL과 선택 가능한 일반 텍스트를 공유하면 원문이
      유실되지 않고 M4 로컬 AI 입력으로 전달된다.
- [ ] 설정을 음성 질문·AI 채팅으로 각각 바꿔 Safari·메모의 같은 텍스트와
      URL, 사진, 지원 문서를 공유하면 자동 듣기 여부만 달라지고 두 경우
      모두 질문에서 공유 자료를 문맥 또는 원본 첨부로 사용한다.
